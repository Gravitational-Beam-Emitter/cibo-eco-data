"""
FastAPI REST API — serve Korean stock market data.

Usage:
    python -m uvicorn kr_stock.api:app --host 127.0.0.1 --port 8006

Endpoints:
    GET  /api/v1/health              — Service health
    GET  /api/v1/daily/{date}        — Full daily review
    GET  /api/v1/stocks/{date}       — Significant movers for date
    GET  /api/v1/stock/{code}        — Stock price + mover history
    ...
    GET  /api/v1/flows               — Foreign/institutional flows
    GET  /api/v1/filings             — DART filings
    GET  /api/v1/filings/{rcept_no}  — Single filing detail
    GET  /api/v1/fetch/status        — Fetch log
    POST /api/v1/fetch               — Trigger daily fetch
    POST /api/v1/fetch-filings       — Trigger DART fetch
...
app = FastAPI(
    title="Korean Stock API",
    description="KOSPI/KOSDAQ/KONEX stock market data (listings, prices, indices, DART filings)",
    version="0.1.0",
)
...
def list_indices(
    index_code: Optional[str] = Query(None, description="KS11 or KQ11"),
...
def foreign_flows(
    ...
    conn = init_db(read_only=True)
    try:
        df = get_foreign_flows(conn, market=market, start=start, end=end, limit=limit)
        records = df.to_dict(orient="records")
        _serialize_records(records)
...
def list_filings(
    corp_name: Optional[str] = Query(None, description="Company name filter"),
    ...
    conn = init_db(read_only=True)
    try:
        df = get_dart_filings(conn, corp_name=corp_name, report_type=report_type,
                               start=start, end=end, limit=limit)
        records = df.to_dict(orient="records")
        _serialize_records(records)
        return {"count": len(records), "filings": records}
    finally:
        conn.close()


@app.get("/api/v1/filings/{rcept_no}")
def filing_detail(rcept_no: str):
    conn = init_db(read_only=True)
    try:
        filing = get_filing_by_rcept_no(conn, rcept_no)
        if filing is None:
            raise HTTPException(status_code=404, detail=f"Filing {rcept_no} not found")
        return filing
    finally:
        conn.close()
...
@app.post("/api/v1/fetch-filings")
def trigger_filings_fetch(date: Optional[str] = None):
    """Trigger DART filings fetch."""
    count = pipeline_fetch_filings(date)
...
def startup():
    init_db()
    logger.info("Korean Stock API started")
