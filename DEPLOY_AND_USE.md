...
全球宏观经济数据平台，涵盖 **21 个数据源、287 指标**，支持 MCP / REST API / Python SDK 三种接入方式。
...
Agent 调用 `tools/list` 看到 7 个工具：

| 工具 | 用途 |
|---|---|
| `data_sources` | **第一步** — 浏览 20 个数据源及其描述 |
...
```
1. data_summary        → 了解全貌
2. search_indicators   → 按主题搜索（如 "CPI"、"Polymarket"、"GitHub Stars"）
